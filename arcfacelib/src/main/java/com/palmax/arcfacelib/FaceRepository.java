...
import com.palmax.arcfacelib.util.ImageUtil;
...
public class FaceRepository {
     ...
     * @return 激活结果code
     */
    public static int activeEngine(Context context){
        return  new FaceEngine().active(context.getApplicationContext(), Constants.APP_ID, Constants.SDK_KEY);
    }

    public static FaceRepository getInstance() {
        if (sFaceRepository == null) {
            synchronized (FaceRepository.class) {
                if (sFaceRepository == null) {
                    sFaceRepository = new FaceRepository();
                }
            }
        }
        return sFaceRepository;
    }
